...
void UClcBackpackSubsystem::ToggleBackpack()
{
	...
	{
		if (BackpackWidget) BackpackWidget->RemoveFromParent();
		bIsOpen = false;
...
int32 UClcBackpackSubsystem::AddStone(const FClcStoneRuntimeData& StoneData)
{
	...
	UE_LOG(LogTemp, Log, TEXT("[ClcBackpack] Added stone '%s' at slot %d. Total: %d"), *StoneData.DisplayName, NewIndex, Stones.Num());
	return NewIndex;
}
...
bool UClcBackpackSubsystem::RemoveStone(int32 StoneIndex)
{
	...
	for (int32 i = StoneIndex; i < Stones.Num(); ++i) Stones[i].BackpackIndex = i;
	if (BackpackWidget && bIsOpen) BackpackWidget->RefreshDisplay(Stones, Gold);
	return true;
}
...
{
	if (Stones.IsValidIndex(Index)) return &Stones[Index];
	return nullptr;
}

void UClcBackpackSubsystem::UpdateStoneData(int32 Index, const FClcStoneRuntimeData& UpdatedData)
{
	if (Stones.IsValidIndex(Index)) { Stones[Index] = UpdatedData; Stones[Index].BackpackIndex = Index; }
}

void UClcBackpackSubsystem::ShowNotification(const FString& Message)
{
	if (GEngine) GEngine->AddOnScreenDebugMessage(-1, 2.5f, FColor::Yellow, Message);
}

void UClcBackpackSubsystem::GMAddGold(int32 Amount)
{
	Gold += Amount;
	if (GEngine) GEngine->AddOnScreenDebugMessage(-1, 3.0f, FColor::Green, FString::Printf(TEXT("[GM] Added %d gold. Total: %d"), Amount, Gold));
}