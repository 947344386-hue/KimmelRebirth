...
void AClcOpeningTool::OnUpdate(const FClcToolTraceInfo& TraceInfo)
{
	...
	if (TraceInfo.bHasHit)
	{
		...
		bHasTarget = true;
		LastFaceIndex = TraceInfo.FaceIndex;
	}
	else
	{
		bHasTarget = false;
...
bool AClcOpeningTool::ExecuteGrind(const FVector& RayOrigin, const FVector& RayDirection)
{
	...
	if (LastFaceIndex >= 0 && LastFaceIndex * 3 + 2 < (int32)LOD.IndexBuffer.GetNumIndices())
	{
		...
		const uint32 I0 = LOD.IndexBuffer.GetIndex(LastFaceIndex * 3);
		const uint32 I1 = LOD.IndexBuffer.GetIndex(LastFaceIndex * 3 + 1);
		const uint32 I2 = LOD.IndexBuffer.GetIndex(LastFaceIndex * 3 + 2);
	...
	// O(N) 回退路径：FaceIndex 无效或 O(1) 未命中时遍历全量三角形
	if (BestTri < 0)
	{
		const FTransform& MeshTM = StoneMeshComp->GetComponentToWorld();
		const FVector LocalRayOrigin = MeshTM.InverseTransformPosition(RayOrigin);
